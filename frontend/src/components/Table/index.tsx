...
const APTable: React.FC<TableProps> = (props) => {
  ...
  return (
    <TableContainer
      component={Paper}
      elevation={1}
      style={{ background: "#49484605", padding: "0px 16px" }}
    >
      <Table>
        <TableHead>
          <TableRow>
            {columns.map((column) => (
              <TableCell align="center" key={column.field}>
                <strong style={{ color: "var(--gray-4)" }}>
                  {column.headerName}
                </strong>
              </TableCell>
            ))}
            <TableCell align="center">
              <strong style={{ color: "var(--gray-4)" }}>Opções</strong>
            </TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
        ...
        <NoFinalBorder>
          <TableRow>
            <TablePagination
              ...
              page={0}
              rowsPerPage={10}
              labelDisplayedRows={(pag) => (
                <span>
                  {pag.from}-{pag.to} de {pag.count}
                </span>
              )}
              rowsPerPageOptions={[]}
              variant="footer"
